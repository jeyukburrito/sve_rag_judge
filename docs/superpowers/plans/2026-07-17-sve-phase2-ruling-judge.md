...
**Goal:** 카드 DB(13,903장)와 공식 Q&A(2,699건)를 인덱싱하고, 질문마다 룰북+카드+Q&A 세 소스를 종합 검색해 판정하는 저지로 확장한다.
...
- Produces:
  ...
  - CLI: `python -m src.build_phase2_index [--db]` → `cards`(13,903), `qna`(2,699) 컬렉션. Task 3이 이 컬렉션을 읽는다.
...
Expected: bge-m3 행의 PROCESSOR에 GPU 표기(예: "100% GPU"). CPU만 나오면 멈추지 말고 보고에 기록만 하고 진행(시간이 40~60분으로 늘어남).

Run: `python -m src.build_phase2_index`
Expected: "카드 13903건 로드" → 진행 카운터 → "Q&A 2699건 로드" → 진행 카운터 → "완료: cards=13903, qna=2699 → ...index"
...
"
```
Expected: counts 13903/2699. 한국어 '로즈퀸' 쿼리에 ローズクイーン 카드가 상위권(정확한 순위는 무관, 3위 내), Q&A는 팡파르(ファンファーレ) 관련 항목이 상위. 전혀 무관하면 멈추고 실제 top-3와 함께 보고.
...
```markdown
- 2단계 (완료): 카드 DB(13,903장) + 공식 Q&A(2,699건) 종합 판정 저지
